...
public class ProjectileManager {
    ...
    public void draw(Graphics g) {
        ...
        drawExplosions(g2d);//patlama animasyonunu cizdirme
    }
    ...
    public void update() {
        for (Projectile p : projectiles) {
            if(p.isActive()){
                ...
                }else if(isProjOutsudeBounds(p)){//atilan bir merminin dusmana carpmayip map disina ciktiginda yok olmasini sagliyor
                    p.setActive(false);
                }
            }
        }
        for(Explosion e :  explosions){//patalam animasyonu
            if(e.getIndex() < 7){
                e.update();
            }
        }
    ...
    private void explodeOnEnemies(Projectile p) {//80 pixel yaricapinda alan hasari vuruyor
        for(Enemy e : playing.getEnemyManager().getEnemies()){
            if(e.isAlive()){
                ...
                if(realDist <= radius){
                    e.hurt(p.getDmg(),p.getProjectileType());
                }
            }
        }
    }
    ...
    private boolean isProjOutsudeBounds(Projectile p) {
        if(p.getPos().x >= 0){
            if(p.getPos().x <= 640){
                if(p.getPos().y >= 0){
                    if(p.getPos().y <= 800){
